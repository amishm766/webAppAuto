...
def action(appliance, action):
    ...
    for socks in read_sockets: 
        ...
        if socks == s: 
            ...
            print(message)
            appliance = raw_input("enter appliance name : ")
            
            actuator = raw_input("enter actuator : ")
            action = raw_input('input action : ')

    """
            
    """ check for which appliance the request is created from webpage """
    #actuator=led[0] if appliance =='led0' else actuator=led[1] if appliance=='led1' else actuator=led[2] if appliance=='led2' else actuator=led[3] if appliance == 'led3'
    if appliance == 'led0':
        actuator = led[0]
    if appliance == 'led1':
        actuator = led[1]
